...
rcbd_dfw <- rcbd_df %>%
  mutate(rep = factor(rep),
         ...
         ) %>% # you are feeding the chunk between this pipe and the other one
...
ggplot(rcbd_dfw, aes(x = nrate_kgha, 
                    y = yield_kgha,
                    color = nrate_kgha)) +
  geom_boxplot() + # we can add geom_point() to have 
  geom_jitter() +
...
ggplot(rcbd_dfw, aes(x = nrate_kgha, 
  ...
  geom_boxplot() +
  geom_jitter() +
  #facet_grid(.~krate_kgha) + #.~ take into account the 
  theme(legend.position = "none")
```
...
rcbd_means_all <- emmeans(rcbd_mod,~krate_kgha|nrate_kgha) #nrate_kgha|krate_kgha for means inside the 2nd one
...
geom_label(data = rcbd_cld_selected,
          ...
          ) + # This will help shrink the letter on the plot in case you have small box
