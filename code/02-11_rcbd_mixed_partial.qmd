...
```{r data import}
rcbd_df <- read_csv("../Downloads/data/wheat_nk_balkh.csv")
...
# Model fitting
rcbd_mix_mod <- lmer(yield_kgha~nrate_kgha*krate_kgha + (1|rep), data = rcbd_dfw, ) # to nest a random factor you can 1|location/rep or location/year/rep to nest rep in year and location
...
```{r randeff_rep}
randeff_rep <- ranef(rcbd_mix_mod)[[1]] # you may use the [[1]] or you can specify the name
...
library(emmeans)
rcbd_mix_means_all <- emmeans(rcbd_mix_mod,~krate_kgha|nrate_kgha)
