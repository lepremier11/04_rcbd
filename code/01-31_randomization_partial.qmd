...
Now, let's plot it.
```{r crd map}
crd_plot <-ggplot(data = crd_layout,
       aes(x = row, y = -col)
       ) +
...
Now, let's plot it.
```{r rcbd map}
rcbd_plot<-ggplot(data = rcbd_layout, aes(x = row, 
                               y = -col))+
  geom_tile(aes(fill = trtname),
            color = "black",
            show.legend = F
...
```{r}
rcbd_rand %>%
  # Retrieving other information contained in trts, important in merging two datasets with one common column
  left_join(trts) %>%
  # Rearranging columns 
  dplyr::select(plot, rep, trtname, trtnum, nrate_kgha,
                krate_kgha) %>%
...
ggsave("../output/rcbd_layoutmap.png",
       ...
       height = 4,
       bg = "white")
ggsave("../output/rcd_layoutmap.png", plot = crd_plot,
       width = 6,
       height = 4,
       bg = "white")
```
